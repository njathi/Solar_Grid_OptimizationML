...
with col2:
    ...
    load_file = st.file_uploader("Upload Load Profile (CSV with datetime and load columns)", type=["csv"])
    use_synthetic_load = st.checkbox("Use synthetic load profile (if no file uploaded)", value=True)
        ...
        try:
            ...
            for col in load_df.columns:
                if 'time' in col.lower() or 'date' in col.lower():
                    datetime_col = col
                if 'load' in col.lower() or 'power' in col.lower() or 'demand' in col.lower():
                    load_col = col
            if datetime_col and load_col:
                ...
                st.success(f"Successfully loaded load profile with {len(load_profile)} data points")
            else:
                st.error("Could not identify datetime and load columns. Please ensure your CSV has columns with 'time'/'date' and 'load'/'power'/'demand' in their names.")
                load_profile = None
        except Exception as e:
            st.error(f"Error loading load profile: {e}")
            load_profile = None
    elif use_synthetic_load:
        st.info("Using synthetic load profile")
        load_profile = None
    else:
        st.warning("Please upload a load profile or check 'Use synthetic load profile'")
        load_profile = None
